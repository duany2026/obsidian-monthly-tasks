...
| 设置项 | 说明 | 默认值 |
|--------|------|--------|
| 已完成任务显示方式 | 已完成任务在月历中的展示样式（带删除线/普通文字/隐藏） | 带删除线 |
| 显示农历 | 是否显示农历日期和节气 | ✅ 开启 |
| 显示节假日 | 是否标注法定节假日和调休 | ✅ 开启 |
| 刷新节假日数据 | 从网络 API 重新获取当年节假日数据 | — |
...
- ✨ 已完成任务显示方式改为三选一下拉菜单（带删除线 / 普通文字 / 隐藏）
- ✨ 新增「刷新节假日数据」按钮（从 timor.tech API 重新获取）
- ✨ 插件启用时自动刷新当年节假日数据（静默执行，不阻塞加载）
- ✨ 设置面板全面适配暗色模式（使用说明、功能特性、快捷键、存储说明四大区域）
- 🎨 每周起始日默认值改为周日
...
**Made with ❤️ by [duany](https://github.com/duany2026)**
